...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
  ...
  private lazy var communityHTTP = BoundedCommunityHTTPClient(
    userAgent: "CodexSkinStudio/\(appVersion)"
  )
  ...
  private func configureStatusItem() {
    ...
    mark.accessibilityDescription = "Codex Skin Studio"
    button.image = mark
    button.toolTip = "Codex Skin Studio"
    rebuildMenu()
  }
  ...
  private func savedThemes() -> [SavedThemeOption] {
    ...
    // "CodexSkinStudioMenuBar"'` 可直接定位是哪一层过滤把主题吃掉了。
  ...
  private func refreshStatus() {
    ...
    ScriptRunner.run(script: script, arguments: ["--json"]) { [weak self] result in
      ...
      if result.succeeded,
        ...
        self.statusItem.button?.toolTip = "Codex Skin Studio · \(parsed.title)"
  ...
  @objc private func toggleLoginItem() {
    do {
      ...
      if SMAppService.mainApp.status == .requiresApproval {
        showInfo(
          title: "需要系统确认",
          message: "请在“系统设置 → 通用 → 登录项”中允许 Codex Skin Studio。"
        )
      }
    } catch {
      showError(
  ...
  @objc private func restoreAndUninstall() {
    ...
    ScriptRunner.run(
      ...
      self.showInfo(
        title: "恢复完成",
        message: "本地引擎和登录启动已移除。最后请把“Codex Skin Studio.app”移到废纸篓。"
      )
      NSApp.terminate(nil)
    }
  }
  ...
  private func legacyPluginURLs() -> [URL] {
    var candidates = [
      stateRootURL.appendingPathComponent("menubar/codex_skin_studio.10s.sh")
    ]
    if let pluginDirectory = UserDefaults(suiteName: "com.ameba.SwiftBar")?
      ...
      candidates.append(
        URL(fileURLWithPath: pluginDirectory, isDirectory: true)
          .appendingPathComponent("codex_skin_studio.10s.sh")
      )
    }
    var seen = Set<String>()

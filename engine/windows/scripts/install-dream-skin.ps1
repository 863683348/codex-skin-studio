...
try {
  ...
  if (Test-DreamSkinTrayActive) {
    throw 'Exit the Codex Skin Studio tray before reinstalling so every shortcut can move to the new runtime safely.'
  }
  ...
  if (-not $NoShortcuts) {
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      $shortcut = $shell.CreateShortcut((Join-Path $folder 'Codex Skin Studio.lnk'))
      ...
      $shortcut.Description = 'Launch the official Codex app with Codex Skin Studio'
      $shortcut.Save()
    }

    $restore = $shell.CreateShortcut((Join-Path $desktop 'Codex Skin Studio - Restore.lnk'))
    ...
    foreach ($folder in @($desktop, $startMenu)) {
      $tray = $shell.CreateShortcut((Join-Path $folder 'Codex Skin Studio - Tray.lnk'))
      ...
      $tray.Description = 'Open Codex Skin Studio status and theme controls in the system tray'
      $tray.Save()
    }
  ...
  if ($NoShortcuts) {
    Write-Host "Codex Skin Studio base theme installed at $($engine.Root). Run $($engine.Start) to launch it."
  } else {
    Write-Host 'Codex Skin Studio installed. The launch shortcut asks before restarting an open Codex window.'
  }
} finally {

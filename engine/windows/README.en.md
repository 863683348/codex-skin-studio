...
Codex Skin Studio loads an external theme into the official Codex Windows desktop app through loopback CDP. The native sidebar, project picker, task content, and composer remain interactive. The tool does not modify WindowsApps, `app.asar`, or the app signature.
...
- `Codex Skin Studio`: launch or reapply the skin.
...
The `Codex Skin Studio` shortcut is the recommended launcher. It asks for confirmation before restarting an open Codex window.
...
Open `Codex Skin Studio - Tray` to:
...
Launch Codex through the `Codex Skin Studio` shortcut, then run verification. A normal Codex launch does not open the debug session used by Dream Skin.
